...
def list_mtp_devices():
    ...
    mtp_namespace = shell.NameSpace(17)  # MTP Namespace
    devices = []

    for device in mtp_namespace.Items():
...
def getFullPathOfMtpDir(mtpDir):
...
async def get_files_from_mtp(device_id, subdir_name, progress_label):
    shell = win32com.client.Dispatch("Shell.Application")
    mtp_namespace = shell.NameSpace(17)
...
async def get_folder_from_mtp(destination):
    shell = win32com.client.Dispatch("Shell.Application")

    # Define the destination path
    full_path = os.path.abspath(destination)
    os.makedirs(full_path, exist_ok=True)
    dest_folder = shell.NameSpace(full_path)

    return dest_folder

async def copy_file_from_mtp(file, mtp_folder):
    if mtp_folder:
        mtp_folder.CopyHere(file)

# Copy Files from MTP Device
async def copy_files_from_mtp(device_id, subdir_name, destination, progress_bar, progress_label):
    shell = win32com.client.Dispatch("Shell.Application")
    mtp_namespace = shell.NameSpace(17)
...
def copy_files_with_progress(items, dest_folder, total_files, progress_bar, progress_label):
...
def list_subdirectories(device_id):
    shell = win32com.client.Dispatch("Shell.Application")
    mtp_namespace = shell.NameSpace(17)
...
def copy_folder_contents(folder, destination):
    os.makedirs(destination, exist_ok=True)
...
def list_files_in_subdirectory(device_id, subdir_name, notification_label):
    shell = win32com.client.Dispatch("Shell.Application")
    mtp_namespace = shell.NameSpace(17)
