...
def show_assign_dialog(db_path_or_conn, astro_object_row, on_done=None):
    ...
    with ui.dialog() as dialog, ui.card().style('width: 600px; max-width: none'):
        ui.label(f"Assign DSO to: {astro_object_row[1]}")

        search_input         = ui.input(label='Search (designation, name, constellation, type)').classes('w-full')
        constellation_filter = ui.input(label='Constellation (exact)').classes('w-full')
        type_filter          = ui.input(label='Type (exact)').classes('w-full')
        dso_select           = ui.select({}, label='Select DSO').classes('w-full')
        custom_dso_input     = ui.input(label='Edit or enter custom description',
